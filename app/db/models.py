...
class Application(Base):
    ...
    __table_args__ = (
        # Index for filtering by status and created_at (for processing queues)
        Index('ix_applications_status_created', 'status', 'created_at'),
        # Index for filtering by job_post_id and status (for job-specific queries)
        Index('ix_applications_job_post_status', 'job_post_id', 'status'),
        # Index for filtering by email and created_at (for user history)
        Index('ix_applications_email_created', 'email', 'created_at'),
        # Index for filtering by status and updated_at (for monitoring recently updated)
