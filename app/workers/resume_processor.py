...
def process_resume(application_id: uuid.UUID):
    ...
    try:
        db = SessionLocal()
        app = (
            db.query(Application).filter(Application.id == application_id).first()  # type: ignore
        )
        if app is None:
        ...
        # Step 1: Call Textract with retries for transient failures
        ...
        try:
            ...
            app.failed_reason = "Failed to start Textract job"
        ...
        try:
            ...
            app.failed_reason = "Grouping failed"
            app.status = ApplicationStatus.FAILED
            db.commit()
            raise
        logger.info("Grouping complete.")

        # Step 3: Call the advanced Gemini service for final processing
        logger.info(
        ...
        # Step 4: Embed the structured data
        ...
        try:
            ...
            app.failed_reason = "Embedding creation failed"
            app.status = ApplicationStatus.FAILED
            db.commit()
            raise
        ...
        try:
            if db is None:
                db = SessionLocal()
            if app is None:
                app = db.query(Application).filter(Application.id == application_id).first()  # type: ignore
